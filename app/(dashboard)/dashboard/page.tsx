...
function QuickAction({
  ...
  return (
    <div
      onClick={onClick}
      className="border-2 border-dashed border-gray-300 rounded-xl p-5 hover:bg-gray-50 cursor-pointer transition"
    >
      <h3 className="font-semibold text-gray-900">{title}</h3>
      <p className="text-sm text-gray-500 mt-1">{description}</p>
    </div>
    ...
    const fetchStats = async () => {
      try {
        ...
        try {
          ...
          totalInventory = items?.length || 0;
        } catch (err) {
          console.error("Failed to fetch inventory:", err);
        }

        setStats({ totalInvoices, totalRevenue, totalCustomers, totalInventory });
      } catch (err) {
        console.error("Error fetching dashboard stats:", err);
      } finally {
        setLoading(false);
