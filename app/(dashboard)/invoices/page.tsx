...
//       const validStatuses = ["draft", "paid", "sent", "overdue", "cancelled"] as const;
...
//         status: ["draft", "paid", "sent", "overdue", "cancelled"].includes(inv.status)
//           ? inv.status
//           : "draft",
//       });
//     } catch (err) {
...
//     // Set custom filename via document.title
//     const customerName = previewInvoice.customer.name.replace(/\s+/g, "_");
//     const invoiceNumber = previewInvoice.invoice_number;
//     document.title = `${customerName}-Invoice-${invoiceNumber}`;
...
//           <div className="bg-white max-w-5xl w-full rounded-lg p-4 overflow-y-auto max-h-[95vh]">
...
//             key={s}
//             onClick={() => setFilter(s)}
//             className={`px-3 py-1 rounded text-sm ${filter === s ? "bg-blue-600 text-white" : "border"}`}
//           >
//             {s.toUpperCase()}
//           </button>
//         ))}
...
//                   <button onClick={() => loadInvoicePreview(inv.id)} className="text-blue-600" title="Preview">
//                     <Printer size={16} />
//                   </button>

//                   <button onClick={() => { setEditingInvoiceId(inv.id); setShowForm(true); }}>
  ...
  const fetchInvoices = async () => {
    ...
    try {
      ...
      for (const docSnap of invoicesSnap.docs) {
        ...
        // ✅ AUTO MARK OVERDUE (but never touch PAID)
          ...
          <tbody>
            {filteredInvoices.map(inv => (
              <tr key={inv.id} className="border-t hover:bg-gray-50">
                ...
                <td className="p-3 flex gap-2">
                  {/* Preview */}
                  <button onClick={() => loadInvoicePreview(inv.id)} className="text-blue-600">
                    <Printer size={16} />
                  </button>
                  ...
                  <button
                    ...
                    className={inv.status === "paid" ? "opacity-40 cursor-not-allowed" : ""}
                  >
                    <Edit2 size={16} className="text-blue-600" />
                  </button>
